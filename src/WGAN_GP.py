...
# Source 1: https://github.com/igul222/improved_wgan_training (Author's code)
# Source 2: https://github.com/hwalsuklee/tensorflow-generative-model-collections
# Source 3: https://github.com/ChengBinJin/WGAN-GP-tensorflow/blob/master/src/wgan_gp.py
...
class WGAN_GP:
    ...
    def __init__(self, model_path, data_path = None, is_training = False,
        ...
        self.clip         = 0.01
        self.n_critic     = 5           # discriminator's iterations
        self.k            = 1.0         #  1.0 is Proposed in the original paper
        self.c            = 1.0         #  1.0 is Proposed in the original paper
        self.lamda        = 0.10        # 10.0 is Proposed in the original paper
                                        # The higher value, the more stable,
                                        # but the slower convergence
        if is_training == True:
            self.train_initialization(hard_load     = hard_load,
                                      di_iterations = self.n_critic,
                                      ge_iterations = 1)
        else:
    ...
    #
    #................................................................................
    def gradient_penalty(self, noise_type = "normal"):
        shape = [self.batch_size, self.image_size, self.image_size, self.channels]
        if noise_type == "uniform":
            ...
            alpha = tf.random.normal(shape = shape,
                                     mean  = 0.0,
                                     stddev= 1.0 * self.c)
        differences  = self.g_output - self.d_input
        interpolates = self.d_input + alpha * differences
        _, diff      = self.discriminator(interpolates, is_training=True, reuse=True)
        gradients    = tf.gradients(diff, [interpolates])[0]
        slopes       = tf.sqrt(tf.reduce_sum(tf.square(gradients), reduction_indices=[1]))
        penality     =  self.lamda * tf.reduce_mean((slopes - self.k) ** 2)
        return  penality
    ...
    def create_optimizer(self):
        ...
        self.ge_loss = -tf.reduce_mean(self.d_fake_logits)
        self.di_loss = -tf.reduce_mean(self.d_real_logits) + tf.reduce_mean(self.d_fake_logits)
        self.di_loss+= self.gradient_penalty(noise_type = "uniform")
        ...
        # Descriminator's weights clipping
        self.di_clip = [p.assign(tf.clip_by_value(p, -self.clip, self.clip)) for p in d_vars]
    ...
    def train(self, max_epoches, show_images = False):
        ...
        for epoch in range(max_epoches):
            for i in range(self.num_batches):
                ...
                for j in range(self.di_iterations):
                     _, _, di_loss = self.session.run([self.di_opt, self.di_clip,self.di_loss],
                                                       feed_dict = {self.d_input: x_batch,
                                                                    self.g_input: z_batch})
                for j in range(self.ge_iterations):
